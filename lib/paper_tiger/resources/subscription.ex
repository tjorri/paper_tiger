defmodule PaperTiger.Resources.Subscription do
  ...
  def list(conn) do
    ...
    filtered_subscriptions =
      ...
      |> Subscriptions.list_namespace()
      |> filter_by_status(Map.get(conn.params, :status))
      |> filter_by_customer(Map.get(conn.params, :customer))
  ...
  # Stripe's status filter has two meta-values that are not literal statuses,
  # plus a documented default. Everything else matches the literal status.
  defp filter_by_status(subscriptions, nil) do
    Enum.reject(subscriptions, &(&1.status == "canceled"))
  end

  defp filter_by_status(subscriptions, status) when is_atom(status) do
    filter_by_status(subscriptions, Atom.to_string(status))
  end

  defp filter_by_status(subscriptions, "all"), do: subscriptions

  defp filter_by_status(subscriptions, "ended") do
    Enum.filter(subscriptions, &(&1.status in ["canceled", "incomplete_expired"]))
  end

  defp filter_by_status(subscriptions, status) do
    Enum.filter(subscriptions, &(&1.status == status))
  end

  defp filter_by_customer(subscriptions, nil), do: subscriptions

  defp filter_by_customer(subscriptions, customer_id) when is_binary(customer_id) do
    Enum.filter(subscriptions, &(&1.customer == customer_id))
  end
  ...
  defp maybe_create_proration_invoice(subscription, params, billable_items_changed?, pre_update_items) do
    ...
    if should_create_proration_invoice?(billable_items_changed?, proration_behavior),
      do: create_proration_invoice(subscription, params, pre_update_items),
      else: subscription
  end
